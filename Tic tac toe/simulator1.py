...
class Player1:
	...
	def computeBlock(self,blockstat):
		...
		for i in xrange(0,3):
			if  blockstat[x][y] == 'x':
				xCount += 1
			elif blockstat[x][y] == 'o':
				yCount += 1
			x = x + 1;
			y = y - 1;
		value += valueFunc(xCount,yCount);
		...
		for row in xrange(0,3):
			...
			for col in xrange(0,3):
				if blockstat[row][col] == 'x':
					xCount += 1
				elif blockstat[row][col] == 'o':
					yCount += 1
			value += valueFunc(xCount,yCount);
	...
	def minimax(self,board,block,old_move,flag,depth,alpha,beta):
		if depth == 0:
			#print "depth is 0"
			return self.computeCell(old_move[0],old_move[1],board)
...
if __name__ == '__main__':
	...
	if len(sys.argv) != 2:
		...
		print '<option> can be 1 => Random player vs. Player3'
		print '                2 => Player3 vs. Human'
